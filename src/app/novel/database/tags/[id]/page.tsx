...
export default async function NovelDatabaseTagDetailPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params
    ...
    const listResult = await listProjectNovel({ page: 1, size: 9, tag: tag.name })
    ...
    return (
        <>
            <NavigationBreadcrumb url="/novel/database" detail={tag.name} />
            <TagDetail data={tag} related={data.list} isAdmin={isAdmin} type={ProjectType.NOVEL} />
        </>
    )
}
