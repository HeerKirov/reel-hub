...
export default async function NovelDatabaseStaffDetailPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params
    ...
    const listResult = await listProjectNovel({ page: 1, size: 9, staff: staff.name })
    ...
    return (
        <>
            <NavigationBreadcrumb url="/novel/database" detail={staff.name} />
            <StaffDetail data={staff} related={data.list} isAdmin={isAdmin} type={ProjectType.NOVEL} />
        </>
    )
}
