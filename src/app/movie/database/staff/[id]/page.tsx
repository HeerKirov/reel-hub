...
export default async function MovieDatabaseStaffDetailPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params
    ...
    const listResult = await listProjectMovie({ page: 1, size: 9, staff: staff.name })
    ...
    return (
        <>
            <NavigationBreadcrumb url="/movie/database" detail={staff.name} />
            <StaffDetail data={staff} related={data.list} isAdmin={isAdmin} type={ProjectType.MOVIE} />
        </>
    )
}
