...
export default async function MangaDatabaseTagDetailPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params
    ...
    const listResult = await listProjectManga({ page: 1, size: 9, tag: tag.name })
    ...
    return (
        <>
            <NavigationBreadcrumb url="/manga/database" detail={tag.name} />
            <TagDetail data={tag} related={data.list} isAdmin={isAdmin} type={ProjectType.MANGA} />
        </>
    )
}
