...
export default async function MangaDatabaseStaffDetailPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params
    ...
    const listResult = await listProjectManga({ page: 1, size: 9, staff: staff.name })
    ...
    return (
        <>
            <NavigationBreadcrumb url="/manga/database" detail={staff.name} />
            <StaffDetail data={staff} related={data.list} isAdmin={isAdmin} type={ProjectType.MANGA} />
        </>
    )
}
