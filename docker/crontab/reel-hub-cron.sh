#!/bin/sh
# 供宿主机 crontab 调用：请求应用 GET /api/cron/tick（需与容器/进程内 SYSCALL_SECRET 一致）
#
# 用法示例（crontab -e）：
#   0 3 * * * /path/to/reel-hub/reel-hub-cron.sh >>/var/log/reel-hub-cron.log 2>&1
# 或显式指定地址（映射端口、反代域名等）：
#   REEL_HUB_BASE_URL=https://example.com SYSCALL_SECRET=xxx /path/to/reel-hub/reel-hub-cron.sh
#
...
SECRET=${SYSCALL_SECRET:-}

if [ -z "$SECRET" ]; then
  echo "reel-hub-cron.sh: 未设置 SYSCALL_SECRET（请在环境变量或 $ROOT/.env 中配置）" >&2
  exit 1
fi
